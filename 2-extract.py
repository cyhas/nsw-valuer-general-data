...
def clean_save(records):
    ...
    if not df.empty:
        ...
        df.to_csv(FINAL_CSV_PATH, index=False, quoting=csv.QUOTE_ALL, mode='a', header=not os.path.exists(FINAL_CSV_PATH))
    else:
...
def process_line(line: str):
    ...
    if line.startswith("C;"):
        ...
        if record:
            processed_record.append(record)
